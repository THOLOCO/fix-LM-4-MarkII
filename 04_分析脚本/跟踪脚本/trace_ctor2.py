...
slot2name={}
for line in open('iat_map.txt'):
    a,n=line.split(None,1)
    n=n.strip()
    if n.startswith('?'):
        slot2name[int(n[1:],16)]=None      # unknown name, but we know the address
    else:
        slot2name[int(a,16)]=n
print("known slots:",sum(1 for v in slot2name.values() if v),"unknown:",sum(1 for v in slot2name.values() if not v))
