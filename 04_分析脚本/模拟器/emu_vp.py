...
def hook(mu,addr,size,user):
    if addr in (0x102a8a15,0x102a8aa7,0x102a8d3b): HIT.append(addr)
    if addr==LZ1:
    ...
    if 0x70000000<=addr<0x70080000:
        ...
        if name=="GetProcAddress":
            ...
            VP.append((args[0],args[1],args[2],ret))
            r=1
        elif rt==-4:   # alloc
...
print()
print("=== VirtualProtect 调用 (%d 次) ===" % len(VP))
for a,s,p,ret in VP:
    print("   lpAddress=%#010x  dwSize=%#08x  flNewProtect=%#04x  (调用点 %#010x)" % (a,s,p,ret-6))
print()
print("=== 三个补丁点命中情况 ===")
for va in (0x102a8a15,0x102a8aa7,0x102a8d3b):
    print("   %#010x: %s" % (va, "★ 执行了" if va in HIT else "  未执行"))
